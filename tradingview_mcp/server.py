...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "(EGX, BIST, NASDAQ, NYSE, Bursa Malaysia, HKEX, SSE, SZSE, SET Thailand, MAI Thailand). "
        ...
        "set_stock_analysis, set_market_overview, set_top_gainers, set_top_losers, set_stock_screener, and more."
    ),
)
...
def top_gainers(exchange: str = "KUCOIN", timeframe: str = "15m", limit: int = 25) -> list[dict]:
    ...
    Args:
        exchange: Exchange name — crypto: KUCOIN, BINANCE, BYBIT, MEXC; stocks: EGX, BIST, NASDAQ, NYSE, BURSA, HKEX, SSE, SZSE, SET, MAI
...
def coin_analysis(symbol: str, exchange: str = "KUCOIN", timeframe: str = "15m") -> dict:
    ...
    Args:
        symbol: Symbol — crypto: "BTCUSDT", "ETHUSDT"; stocks: "COMI" (EGX), "THYAO" (BIST), "600519" (SSE), "300251" (SZSE), "PTT" (SET), "ADVANC" (SET)
        exchange: Exchange — crypto: KUCOIN, BINANCE, MEXC; stocks: EGX, BIST, NASDAQ, NYSE, BURSA, HKEX, SSE, SZSE, SET, MAI
...
# ── SET Thailand market tools ──────────────────────────────────────────────────

@mcp.tool()
def set_stock_analysis(symbol: str, timeframe: str = "1D") -> dict:
    """Full technical analysis for a Thai SET or MAI stock.

    Args:
        symbol: Thai stock symbol — e.g. PTT, ADVANC, CPALL, DELTA, GULF, AOT, KBANK, SCB, BBL
                (supports both SET main board and MAI market stocks)
        timeframe: One of 5m, 15m, 1h, 4h, 1D, 1W, 1M (default 1D for stocks)

    Returns:
        Detailed analysis including RSI, MACD, Bollinger Bands, moving averages,
        buy/sell signal, oscillators summary, and market sentiment.

    Example:
        set_stock_analysis("PTT", "1D")
        set_stock_analysis("DELTA", "1W")
        set_stock_analysis("CPALL", "4h")
    """
    timeframe = sanitize_timeframe(timeframe, "1D")
    return analyze_coin(symbol, "set", timeframe)


@mcp.tool()
def set_market_overview(timeframe: str = "1D", limit: int = 15) -> dict:
    """Get an overview of the Thai Stock Exchange (SET) market — top gainers, top losers, volume leaders.

    Args:
        timeframe: One of 5m, 15m, 1h, 4h, 1D, 1W, 1M (default 1D)
        limit: Number of stocks per category (max 25)

    Returns:
        Market overview with top gainers, top losers, most active by volume,
        and overall market breadth indicators.

    Example:
        set_market_overview("1D", 10)
        set_market_overview("1W", 20)
    """
    timeframe = sanitize_timeframe(timeframe, "1D")
    limit = max(1, min(limit, 25))

    # Gainers
    gainers_raw = fetch_trending_analysis("set", timeframe=timeframe, limit=limit * 2)
    gainers_raw.sort(key=lambda x: x["changePercent"], reverse=True)
    gainers = [
        {"symbol": r["symbol"], "changePercent": r["changePercent"], "indicators": dict(r["indicators"])}
        for r in gainers_raw[:limit]
    ]

    # Losers
    losers_raw = list(gainers_raw)
    losers_raw.sort(key=lambda x: x["changePercent"])
    losers = [
        {"symbol": r["symbol"], "changePercent": r["changePercent"], "indicators": dict(r["indicators"])}
        for r in losers_raw[:limit]
    ]

    # Volume leaders (sort by volume indicator if available)
    vol_raw = list(gainers_raw)
    vol_raw.sort(key=lambda x: x["indicators"].get("volume", 0) or 0, reverse=True)
    volume_leaders = [
        {
            "symbol": r["symbol"],
            "changePercent": r["changePercent"],
            "volume": r["indicators"].get("volume", 0),
            "close": r["indicators"].get("close", 0),
        }
        for r in vol_raw[:limit]
    ]

    total_up = sum(1 for r in gainers_raw if r["changePercent"] > 0)
    total_down = sum(1 for r in gainers_raw if r["changePercent"] < 0)
    total_flat = sum(1 for r in gainers_raw if r["changePercent"] == 0)
    ...
    return {
        ...
        "market_breadth": {
            ...
            "unchanged": total_flat,
            "advance_decline_ratio": round(total_up / max(1, total_down), 2),
        },
        "top_gainers": gainers,
        "top_losers": losers,
...
@mcp.tool()
def set_top_gainers(timeframe: str = "1D", limit: int = 20) -> list[dict]:
    """Return top gaining Thai SET stocks for a given timeframe.

    Args:
        timeframe: One of 5m, 15m, 1h, 4h, 1D, 1W, 1M (default 1D)
        limit: Number of stocks to return (max 50)

    Returns:
        List of top gainers with symbol, % change, RSI, volume, and close price.

    Example:
        set_top_gainers("1D", 20)
        set_top_gainers("1W", 10)
    """
    timeframe = sanitize_timeframe(timeframe, "1D")
    limit = max(1, min(limit, 50))
    rows = fetch_trending_analysis("set", timeframe=timeframe, limit=limit)
    return [
        {"symbol": r["symbol"], "changePercent": r["changePercent"], "indicators": dict(r["indicators"])}
        for r in rows
    ]
...
@mcp.tool()
def set_top_losers(timeframe: str = "1D", limit: int = 20) -> list[dict]:
    """Return top losing Thai SET stocks for a given timeframe.
    ...
    Returns:
        List of top losers with symbol, % change, RSI, and key indicators.

    Example:
        set_top_losers("1D", 20)
        set_top_losers("1W", 10)
    """
    timeframe = sanitize_timeframe(timeframe, "1D")
    limit = max(1, min(limit, 50))
    rows = fetch_trending_analysis("set", timeframe=timeframe, limit=limit * 2)
    rows.sort(key=lambda x: x["changePercent"])
    return [
        {"symbol": r["symbol"], "changePercent": r["changePercent"], "indicators": dict(r["indicators"])}
        for r in rows[:limit]
...
def set_stock_screener(
    ...
    """Screen Thai SET stocks by RSI, price change, and buy/sell signal.

    Args:
        timeframe: One of 5m, 15m, 1h, 4h, 1D, 1W, 1M (default 1D)
        min_rsi: Minimum RSI value (0-100, default 0) — e.g. 30 for oversold filter
        max_rsi: Maximum RSI value (0-100, default 100) — e.g. 70 for not overbought
        min_change: Minimum % price change (default -100, no filter)
        signal: Filter by signal — "buy", "sell", "strong_buy", "strong_sell", "neutral", "any"
        limit: Number of results (max 50)

    Returns:
        Filtered list of Thai stocks matching the criteria.

    Example:
        set_stock_screener("1D", min_rsi=25, max_rsi=45, signal="buy")
        set_stock_screener("1D", min_change=2.0, signal="strong_buy")
        set_stock_screener("1W", min_rsi=60, max_rsi=80)
    """
    timeframe = sanitize_timeframe(timeframe, "1D")
    min_rsi = max(0.0, min(100.0, min_rsi))
    max_rsi = max(0.0, min(100.0, max_rsi))
    limit = max(1, min(limit, 50))
    signal = signal.lower().strip()

    rows = fetch_trending_analysis("set", timeframe=timeframe, limit=200)

    filtered = []
    for r in rows:
        ind = r["indicators"]
        rsi_val = ind.get("RSI", ind.get("rsi", None))
        if rsi_val is None:
            continue
        if not (min_rsi <= rsi_val <= max_rsi):
            continue
        if r["changePercent"] < min_change:
            continue
        bb_signal = str(ind.get("BB_signal", ind.get("signal", ""))).lower()
        if signal != "any" and signal not in bb_signal:
            continue
        filtered.append({
            "symbol": r["symbol"],
            "changePercent": r["changePercent"],
            "rsi": round(rsi_val, 2),
            "signal": ind.get("BB_signal", ind.get("signal", "N/A")),
            "close": ind.get("close", ind.get("Close", None)),
            "volume": ind.get("volume", ind.get("Volume", None)),
        })
...
@mcp.tool()
def set_multi_timeframe(symbol: str) -> dict:
    """Multi-timeframe alignment analysis for a Thai SET stock (Weekly → Daily → 4H → 1H → 15m).

    Args:
        symbol: Thai stock symbol — e.g. PTT, ADVANC, CPALL, DELTA, GULF, KBANK

    Returns:
        Trend alignment across all timeframes to identify confluence zones.

    Example:
        set_multi_timeframe("PTT")
        set_multi_timeframe("DELTA")
    """
    full_symbol = f"SET:{symbol.upper()}" if ":" not in symbol else symbol.upper()
    return run_multi_timeframe_analysis(full_symbol, "set")
...
def multi_timeframe_analysis(symbol: str, exchange: str = "KUCOIN") -> dict:
    ...
    Args:
        symbol: Symbol — crypto: "BTCUSDT"; stocks: "COMI" (EGX), "THYAO" (BIST), "600519" (SSE), "300251" (SZSE), "PTT" (SET), "ADVANC" (SET)
        exchange: Exchange — crypto: KUCOIN, BINANCE, MEXC; stocks: EGX, BIST, NASDAQ, NYSE, SSE, SZSE, SET, MAI
    """
    exchange = sanitize_exchange(exchange, "KUCOIN")
    full_symbol = symbol.upper() if ":" in symbol else f"{exchange.upper()}:{symbol.upper()}"
...
@mcp.tool()
def set_stock_financial(symbol: str) -> dict:
    """Get full financial statements for a Thai SET stock — income, balance sheet, cash flow.

    Fetches data from Yahoo Finance (symbol.BK format) and returns:
      - current_metrics: P/E, EPS, ROE, ROA, profit margin, debt/equity, market cap
      - quarterly_income: last 4 quarters — revenue, gross profit, net income, EPS
      - annual_income:    last 4 years — revenue, net income, EPS
      - quarterly_balance: last 4 quarters — total assets, equity, debt
      - quarterly_cashflow: last 4 quarters — operating/investing/financing CF, free CF
      - eps_trend: EPS estimates vs actuals

    All monetary values are in million THB (ล้านบาท).

    Args:
        symbol: Thai SET stock symbol — e.g. PTT, ADVANC, CPALL, FORTH, KBANK, SCB, AOT

    Example:
        set_stock_financial("PTT")
        set_stock_financial("ADVANC")
